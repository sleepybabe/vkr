function checkCriterion1() {
    ...
    if (absoluteResult.snapshotLength === 0 || relResult.snapshotLength === 0 || isAltNotFoundRel || isAltNotFoundAbs)
        return "1 задание: не выполнено. Отсутствуют изображения с альтернативным текстом и с абсолютной и относительной url"
...
function checkCriterion2() {
    ...
    if (!images || isNotRatio || isNotSameSize)
        return "2 задание: не выполнено. Отсутствуют три разных по размеру изображения с одинаковым url, сохраняющие пропорции."
...
function checkCriterion3() {
    ...
    if (result.snapshotLength === 0)
        return "3 задание: не выполнено. Отсутствуют ссылка с одного из изображений."
...
function checkCriterion4() {
    ...
    if (!isConnected || isNotHrefInArea)
        return "4 задание: не выполнено. Отсутствуют ссылка с одного из изображений."
...
checkCriteria(checkCriterion1, checkCriterion2, checkCriterion3, checkCriterion4)